...
pub struct LiteMarket {
    ...
    probability: f64,
    /// For CPMM markets, the number of shares in the liquidity pool. For DPM markets, the amount of mana invested in each answer.
    pool: MarketType,
    ...
    /// PSEUDO_NUMERIC markets only, the current market value, which is mapped from probability using min, max, and isLogScale.
    value: Option<f64>,
    /// PSEUDO_NUMERIC markets only, the minimum resolvable value
    min: Option<f64>,
    /// PSEUDO_NUMERIC markets only, the maximum resolvable value
    max: Option<f64>,
    /// PSEUDO_NUMERIC markets only, if true `number = (max - min + 1)^probability + minstart - 1`, otherwise `number = min + (max - min) * probability`
